    ...
    if request.method == 'POST':
        ...
        if s_item_object:
            if current_user.can_sell(s_item_object):
                ...
                flash(f"Unfortunately, something wwent wrong with selling {s_item_object.name}", category='danger')
...
def register_page():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('market_page'))
    if form.errors is not {}: #if there are no errors from the validators
